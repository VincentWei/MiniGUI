...
/* Visual LR */
//static char* buf = "\xe4\xe5""test";

static char* buf = "ab""\xe6\xe7""cd";
//static char* buf = "\xe4\xe5""ab""\xe6\xe7""cd";
//static char* buf = "\xe4\xe5\xe6\xe7";
//static char* buf = "\xe4\xd8\xc2\xd1\xd5\xe4\xc2\xe4\xc7"
//                   "this is only a test.";
//static char* buf = "this";

static char tip[100] = "this is only a test.";

static PLOGFONT  edit_font;
static BOOL hebrew_input = TRUE;
...
static void set_keyboard_layout(HWND hDlg)
{
    if (!hebrew_input) {
        hebrew_input = TRUE;
        strcpy(tip, "Hebrew Input:");
        SetKeyboardLayout (KBD_LAYOUT_HEBREW);
    }
    else {
        hebrew_input = FALSE;

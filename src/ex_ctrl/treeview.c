/*
** $Id: treeview.c 13674 2010-12-06 06:45:01Z wanzheng $
** 
...
#define SYSICON_FOLD "treefold"
#define SYSICON_UNFOLD "treeunfold"
#define BKC_HILIGHT_NORMAL WE_BGC_HIGHLIGHT_ITEM
...
void TreeViewControlCleanup (void)
{
    ...
    if (icon_fold) DestroyIcon (icon_fold);
    if (icon_unfold) DestroyIcon (icon_unfold);
}
...
{    
    ...
    if (pData->dwStyle & TVS_WITHICON) {
        ...
        if (root_ii && root_ii->hIconUnfold)
            ...
            tvRoot->hIconUnfold = ICON_UNFOLD;
    }
 ...
 * draw all items.
 */
static void doAll (HWND hWnd, HDC hdc, PTVDATA pData, PTVITEM p, int cenX, int cenY, int* count, int* depth)
{
        ...
        case MSG_SIZECHANGED:
            {
                ...
                //recalculate scrollbar information
                pData->nVisItemCount = getVisItemCount (pData);
                tvSetVScrollInfo (hwnd, pData, TRUE);
                tvSetHScrollInfo (hwnd, pData, TRUE);
            }
                ...
                switch (LOWORD (wParam)) {
                    case SCANCODE_CURSORBLOCKRIGHT:
                        ...
                        if (p->child == NULL)
                            ...
                            pData->nWidth = getTVWidth (pData);
                            pData->nVisItemCount = getVisItemCount (pData);
                            fprintf(stderr, "VisItemCount is %d.\n", pData->nVisItemCount);
                    ...
                    case SCANCODE_CURSORBLOCKLEFT:
                        ...
                        if (p->child && !(p->dwFlags & TVIF_FOLD)) {
                            ...
                            pData->nWidth = getTVWidth (pData);
                            pData->nVisItemCount = getVisItemCount (pData);
                            fprintf(stderr, "Left:VisItemCount is %d.\n", pData->nVisItemCount);
                            count = count - pData->nItemTop - 1;
                            rc.top = rctop + count * pData->nItemHeight;
                            if (pData->dwStyle & WS_BORDER)
                                rc.top += TV_BORDER;

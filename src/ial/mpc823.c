/*
** $Id: mpc823.c 10951 2008-09-11 03:51:14Z xwyan $
**
...
{
    ts = open ("/dev/tpanel", O_RDONLY);
    if (ts < 0) {
        fprintf (stderr, "MPC823: Can not open touch screen!\n");
        //return FALSE;
    }

    kb_fd = open ("/dev/kb_7289", O_RDONLY);
    if (kb_fd < 0 ) {
        fprintf (stderr, "MPC823: Can not open key board!\n");
        //return FALSE;
    }
    if (ioctl(kb_fd, KDSKBMODE, K_MEDIUMRAW) < 0) {
        fprintf (stderr, "MPC823: key board ioctl error!\n");
        //return FALSE;
    }

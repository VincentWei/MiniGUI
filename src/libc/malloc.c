...
* Synopsis of compile-time options:
    ...
    WIN32                      NOT defined
    MALLOC_HAVE_MEMCPY                defined
    USE_MEMCPY                 1 if MALLOC_HAVE_MEMCPY is defined
    MALLOC_HAVE_MMAP                  defined as 1 
    MMAP_CLEARS                1
    MALLOC_HAVE_MREMAP                0 unless linux defined
    malloc_getpagesize         derived from system #includes, or 4096 if not
    MALLOC_HAVE_USR_INCLUDE_MALLOC_H  NOT defined
...
#define MALLOC_HAVE_MMAP            0
#define MMAP_CLEARS                 0
#define MALLOC_HAVE_MREMAP          0

#define LACKS_UNISTD_H
#define LACKS_SYS_PARAM_H
#define LACKS_SYS_MMAN_H
#undef MALLOC_HAVE_GETPAGESIZE

#define USE_MALLOC_LOCK             1

#define MORECORE                    __rtos_MoreCore
#define MORECORE_CONTIGUOUS         1
#define MORECORE_FAILURE            ((void*)(-1))
#define MORECORE_CANNOT_TRIM

static unsigned char* __rtos_heap;
static unsigned int __rtos_heap_size;
static unsigned int __rtos_free;
static int (*__rtos_mutex_lock) (void);
static int (*__rtos_mutex_unlock) (void);

static void* __rtos_MoreCore (int size)
{
    void* start;

    if (size >= 0 && __rtos_free + size <= __rtos_heap_size) {
        start = __rtos_heap + __rtos_free;
        __rtos_free += size;
...
{
    if (heap == NULL || heap_size < 1024 
            || lock_heap == NULL || unlock_heap == NULL)
        return -1;

    __rtos_heap = heap;
    __rtos_heap_size = heap_size;
    __rtos_mutex_lock = lock_heap;
    __rtos_mutex_unlock = unlock_heap;

    return 0;
}

#endif /* __NOUNIX__ && !WIN32 */
...
/*
  MALLOC_HAVE_MEMCPY should be defined if you are not otherwise using
...
#define MALLOC_HAVE_MEMCPY

#ifndef USE_MEMCPY
#ifdef MALLOC_HAVE_MEMCPY
#define USE_MEMCPY 1
#else
#define USE_MEMCPY 0
#endif
...
#if (__STD_C || defined(MALLOC_HAVE_MEMCPY))
...
/*
  Define MALLOC_HAVE_MMAP as true to optionally make malloc() use mmap() to
...
/* 
   ...
   MALLOC_HAVE_MMAP).  The value must be a multiple of page size.  This
...
/*
  Define MALLOC_HAVE_MREMAP to make realloc() use mremap() to re-allocate
  large blocks.  This is currently only possible on Linux with
  kernel versions newer than 1.3.77.
*/

#ifndef MALLOC_HAVE_MREMAP
#ifdef linux
#define MALLOC_HAVE_MREMAP 1
#else
#define MALLOC_HAVE_MREMAP 0
#endif

#endif /* MALLOC_HAVE_MMAP */
...
#  else
#    if defined(BSD) || defined(DGUX) || defined(MALLOC_HAVE_GETPAGESIZE)
...
/*
  ...
  MALLOC_HAVE_USR_INCLUDE_MALLOC_H should be set if you have a
  ...
  MALLOC_HAVE_USR_INCLUDE_MALLOC_H.
*/

/* #define MALLOC_HAVE_USR_INCLUDE_MALLOC_H */

#ifdef MALLOC_HAVE_USR_INCLUDE_MALLOC_H
...
. Some systems have a limited number of internal tables for
  ...
  MALLOC_HAVE_MMAP is not set, the default value is 0, and attempts to set it
...
#define MALLOC_PREACTION   __rtos_mutex_lock()
#define MALLOC_POSTACTION  __rtos_mutex_unlock()
...
/*
   ...
   * If MALLOC_HAVE_MMAP is true, but mmap doesn't support
  ...
  if (mem != 0) {
    ...
    */
    ...
    /*
      ...
      Note that if MALLOC_HAVE_MMAP is false but chunk_is_mmapped is
...
#else 
    /* If !MALLOC_HAVE_MMAP, but chunk_is_mmapped, user must have overwritten mem */
    check_malloc_state();
    MALLOC_FAILURE_ACTION;
    return 0;
#endif
...
/*
  ...
  request, and MALLOC_HAVE_MMAP is true, then mmap is used as a noncontiguous

/*
**  $Id: st7167_video.h 13059 2010-07-30 07:44:50Z humingming $
**  
**  Copyright (C) 2003 ~ 2010 Feynman Software.
**  Copyright (C) 2001 ~ 2002 Wei Yongming
**
**  All rights reserved by Feynman Software.
**
**  This is the ST7167 platform DirectFB NEWGAL engine.
**
*/

#ifdef _MGGAL_DFB_ST7167

#ifndef _GAL_st7167video_h
#define _GAL_st7167video_h

#include "sysvideo.h"

/* TDDO: define platform device data */


/* TDDO: define platform private_hwdata data */


/* platform callbck function */
int mgChangeDeviceCallback(GAL_VideoDevice*);


#endif /* _GAL_st7167video_h */

#endif /* _MGGAL_DFB_ST7167 */

